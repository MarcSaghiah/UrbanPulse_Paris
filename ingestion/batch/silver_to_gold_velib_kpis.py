...
def run(silver_bucket: str, gold_bucket: str, region: str) -> str:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Build Velib Gold KPI snapshot from latest Silver parquet")
    parser.add_argument("--silver-bucket", default=os.getenv("S3_SILVER_BUCKET", ""))
    parser.add_argument("--gold-bucket", default=os.getenv("S3_GOLD_BUCKET", ""))
    parser.add_argument("--region", default=os.getenv("AWS_REGION", "eu-west-3"))
    args = parser.parse_args()

    if not args.silver_bucket:
        raise ValueError("Missing S3_SILVER_BUCKET")
    if not args.gold_bucket:
        raise ValueError("Missing S3_GOLD_BUCKET")

    run(args.silver_bucket, args.gold_bucket, args.region)
